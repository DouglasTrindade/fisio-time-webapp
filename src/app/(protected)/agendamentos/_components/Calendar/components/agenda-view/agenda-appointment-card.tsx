...
const agendaAppointmentCardVariants = cva(
...
export function AgendaAppointmentCard({ appointment, appointmentCurrentDay, appointmentTotalDays }: IProps) {
  const { badgeVariant } = useCalendar();

  const startDate = parseISO(appointment.startDate);
  const endDate = parseISO(appointment.endDate);

  const color = (badgeVariant === "dot" ? `${appointment.color}-dot` : appointment.color) as VariantProps<typeof agendaAppointmentCardVariants>["color"];

  const agendaAppointmentCardClasses = agendaAppointmentCardVariants({ color });
      ...
      <div role="button" tabIndex={0} className={agendaAppointmentCardClasses} onKeyDown={handleKeyDown}>
        <div className="flex flex-col gap-2">
          ...
          <div className="mt-1 flex items-center gap-1">
            ...
            <p className="text-xs text-foreground">{appointment.user.name}</p>
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            <p className="text-xs text-foreground">{appointment.description}</p>
          </div>
        </div>
      </div>
