...
const COLORS: TAppointment[] = ["blue", "green", "red", "yellow", "purple", "orange", "gray"];
...
const mockGenerator = (numberOfAppointments: number): IAppointment[] => {
  ...
  // Create an appointment happening now
  const currentAppointment = {
    id: String(currentId++),
    startDate: new Date(now.getTime() - 30 * 60000).toISOString(),
    endDate: new Date(now.getTime() + 30 * 60000).toISOString(),
    title: APPOINTMENTS[Math.floor(Math.random() * APPOINTMENTS.length)],
    color: COLORS[Math.floor(Math.random() * COLORS.length)],
    description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
    user: randomUser,
  };

  // Only add the current appointment if it's not on September 20th
  if (now.getMonth() !== 8 || now.getDate() !== 20) {
    // Month is 0-indexed (8 = September)
    result.push(currentAppointment);
  }

  // Generate the remaining appointments
  let i = 0;
  let attempts = 0;
  const maxAttempts = numberOfAppointments * 3; // Prappointment infinite loop with a reasonable max attempts

  while (i < numberOfAppointments - 1 && attempts < maxAttempts) {
    attempts++;

    // Determine if this is a multi-day appointment (10% chance)
    ...
    if (isMultiDay) {
      // Multi-day appointment: Add 1-4 days
      const additionalDays = Math.floor(Math.random() * 4) + 1;
      endDate.setDate(startDate.getDate() + additionalDays);

      // Ensure multi-day appointments don't cross September 20th
      const endMonth = endDate.getMonth();
      const endDay = endDate.getDate();
      const startMonth = startDate.getMonth();
      const startDay = startDate.getDate();

      // Check if appointment spans across September 20th
      if (
...
export const CALENDAR_ITEMS_MOCK: IAppointment[] = mockGenerator(80);
